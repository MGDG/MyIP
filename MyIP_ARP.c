...
ARP_CACHE_TYDEF ARP_Cache[10];	//可存放的缓存数量
*/
...
bool ARPCache_Write(const uint8_t *ip,const uint8_t *MAC)
{
	...
	for(uint16_t i=0;i<CacheNum;i++)
	{
		if(ARP_Cache[i].Flg.reg.ActivFlg == 0)
		{
			...
			//检查写入是否正确
			if( (memcmp(ip,ARP_Cache[i].IP,4) == 0) && (memcmp(MAC,ARP_Cache[i].MAC,6) == 0) )
			{
				ARP_Cache[i].Flg.reg.ActivFlg = 1;
				ARP_Cache[i].LiveTime = 0;
				return true;
			}
			else
	...
	//没找到未使用的缓存位置，则将记录时间最长的一个替换掉
	uint32_t MaxTime = ARP_Cache[0].LiveTime;
	uint16_t index = 0;
	for(uint16_t i=1;i<CacheNum;i++)
	{
		if(ARP_Cache[i].LiveTime > MaxTime)
		{
			MaxTime = ARP_Cache[i].LiveTime;
	...
	//检查写入是否正确
	if( (memcmp(ip,ARP_Cache[index].IP,4) == 0) && (memcmp(MAC,ARP_Cache[index].MAC,6) == 0) )
	{
		ARP_Cache[index].Flg.reg.ActivFlg = 1;
		ARP_Cache[index].LiveTime = 0;
		return true;
	}
	else
	{
